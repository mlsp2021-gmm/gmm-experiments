...
function run_experiment(params)
    ...
    mixtures  = [ Gamma(200.0, inv(100.0)), Gamma(400.0, inv(100.0)), Gamma(600.0, inv(100.0)), Gamma(800.0, inv(100.0)) ]
...
function generate_plots(input; with_pgf = true)
    try
        ...
        gen_folder = projectdir("results", "synthetic", "4mixtures", "with_known_shape_rate")
        stats_path = projectdir("results", "synthetic", "4mixtures", "with_known_shape_rate", savename("stats", params, allowedtypes = save_types)) 
        ...
        if with_pgf
            pgf_path = projectdir("results", "synthetic", "tikz", "4mixtures", "with_known_shape_rate", savename("fig", params, "tikz", allowedtypes = save_types))
            # pgf_histogram(string(params["constraint"]), mean(result.mswitch[end]), mean.(result.mas[end]), mean.(result.mbs[end]), dataset)
            pgf_densities(string(params["constraint"]), mixture, result, dataset, pgf_path)

        end

        fig_path = projectdir("results", "synthetic", "4mixtures", "with_known_shape_rate", savename("fig", params, "png", allowedtypes = save_types)) 
...
results = map(experiments) do experiment
    ...
    cache_path  = projectdir("dump", "synthetic", "4mixtures", "with_known_shape_rate")
...
begin
    ...
    pgf_path = projectdir("results", "synthetic", "tikz", "4mixtures", "comparison", savename("comaprison_4_shape_rate_seed_$(em_id)", [], "tikz")) 
