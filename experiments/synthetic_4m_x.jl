...
function generate_plots(input; with_pgf = true)
    try
        ...
        gen_folder = projectdir("results", "synthetic", "4mixtures", "with_known_mixing")
        stats_path = projectdir("results", "synthetic", "4mixtures", "with_known_mixing", savename("stats", params, allowedtypes = save_types)) 
        ...
        if with_pgf
            pgf_path = projectdir("results", "synthetic", "tikz", "4mixtures", "with_known_mixing", savename("fig", params, "tikz", allowedtypes = save_types)) 
            pgf_densities(string(params["constraint"]), mixture, result, dataset, pgf_path)
        end

        fig_path = projectdir("results", "synthetic", "4mixtures", "with_known_mixing", savename("fig", params, "png", allowedtypes = save_types)) 
...
results = map(experiments) do experiment
    ...
    cache_path  = projectdir("dump", "synthetic", "4mixtures", "with_known_mixing")
    ...
    try
        ...
        generate_plots(result, with_pgf = true)
...
# Generate simple plots for each result
...
begin
    ...
    pgf_path = projectdir("results", "synthetic", "tikz", "4mixtures", "comparison", savename("comaprison_4_mixing_seed_$(em_id)", [], "tikz")) 
